...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/type-test'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: TypeTest) {
        return item.id;
    }
    registerChangeInTypeTests() {
        this.eventSubscriber = this.eventManager.subscribe('typeTestListModification', (response) => this.loadAll());
    }
